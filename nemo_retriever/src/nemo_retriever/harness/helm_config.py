...
class HelmDeploymentConfig:
    ...
    def __post_init__(self) -> None:
        ...
        for name in ("helm_timeout", "readiness_timeout", "helm_service_local_port", "service_max_concurrency"):
...
def load_helm_config(path: Path) -> HelmDeploymentConfig:
    ...
    if not repository or not tag:
        raise ValueError(
            "Helm target requires an explicit main/nightly service image via "
