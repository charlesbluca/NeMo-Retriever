...
def invoke_image_inference_batches(
    *,
    ...
    When `merge_levels` is provided it must be the same length as
    `image_b64_list` and is forwarded to the NIM as a per-image
    ``merge_levels`` payload field (values: ``"word"``, ``"sentence"``,
    ``"paragraph"``).
