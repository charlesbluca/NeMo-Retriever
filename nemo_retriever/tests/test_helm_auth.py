...
def test_auth_is_disabled_by_default_and_configmap_has_no_token() -> None:
    documents = _render()
    ...
    config = next(
        ...
        and "retriever-service.yaml" in item.get("data", {})
    )
...
def test_standalone_wires_separate_public_and_internal_secrets() -> None:
    ...
    for component in ("service", "vectordb"):
        internal = _container_env(deployments[component])["NRL_INTERNAL_VDB_TOKEN"][
            "valueFrom"
        ]["secretKeyRef"]
        assert internal == {
    ...
    assert (
        scope_env["value"] == "/var/run/secrets/nemo-retriever/auth/scope-tokens.json"
    )
    pod_spec = service["spec"]["template"]["spec"]
    scope_volume = next(
        item for item in pod_spec["volumes"] if item["name"] == "scope-token"
    )
...
def test_split_mounts_public_secret_only_on_gateway_and_internal_secret_everywhere() -> (
    None
):
    ...
    for component in ("gateway", "realtime", "batch", "vectordb"):
        assert (
            _container_env(deployments[component])["NRL_INTERNAL_VDB_TOKEN"][
                "valueFrom"
            ]["secretKeyRef"]["name"]
            == "nrl-internal-auth"
        )
    ...
    for component in ("realtime", "batch", "vectordb"):
        ...
        assert all(
            item["name"] != "scope-token" for item in pod_spec.get("volumes", [])
        )
